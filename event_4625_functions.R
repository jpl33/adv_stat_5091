  ...
  for (i in 1:length(usr_lst)){
    ...
    for (j in 1:nrow(usr_lst[[i]])){
      ...
      src<-unique(df_bs[ ((as.Date(substr(df_bs[,"Event.Time"],1,10),"%Y/%m/%d") %like% usr_lst[[i]][j,1])
                         ...
                         ,c("sourceHostName","sourceAddress") ])
      if (length(src)==0){
        src<-10
      }
      if(!(class(src)=="numeric")){
        for (k in 1:length(src)){
           r1<-cbind(usr_lst[[i]][j,1],usr_lst[[i]][j,2],src[k,1],src[k,2])
    ...
    if(!(df_src==0)){
        ...
        colnames(df_src)<-c("date","user","srcName","srcAddress")
...
func_4625_file_src<-function(file_nm){
  file1_src<-0
...
func_4625_output<-function(src_lst,file_nm){
  ...
  for(i in 1:length(src_lst)){
    if(!(src_lst[[i]][1,"date"] %in% trgt_dates)){
      write.csv(src_lst[[i]],paste("../output/4625_",src_lst[[i]][1,"date"],".csv",sep = ""),row.names = FALSE)
    } else {
...
func_4625<-function(input_filter){
  ...
  for (i in 1:length(csv)){
    src<-func_4625_file_src(csv[i])
    if (!(class(src)=="numeric")){
      func_4625_output(src,csv[i])}
  }
}
